...
TITULO = "OMNI Agro"
LEMA = "Del sobrevuelo del dron a un censo que se puede auditar"
FRASE = ("Sin seguimiento, una planta vista en 40 fotogramas serían 40 plantas. Ese paso convierte detecciones en un censo.")
...
def main() -> int:
    med = medidas()
    y11 = med.get("uav_weed_yolo26.pt", {})
    ...
    PASOS = [
        ("01", "dron", "Vuela", "Video cenital del dron",
         f"0 €", f"en sensores nuevos"),
        ("02", "planta", "Detecta", "YOLO26 afinado para dron",
         f"{acierto}", f"mAP@50 · umbral 0.05"),
        ("03", "ruta", "Sigue", "ByteTrack pone el ID",
         f"1", f"ID por planta"),
        ("04", "grafico", "Cuenta", "Censo, maleza y huecos",
         f"3", f"cifras que hoy se estiman"),
    ]
    ...
    n = len(med)
    for i, (et, val) in enumerate((("Pruebas", f"15 comprobaciones"), ("Modelos medidos", f"{n}"), ("Corre en", f"CPU o GPU"), ("Datos simulados", f"ninguno"))):
        x = m + i * ((W - 2 * m) / 4)
