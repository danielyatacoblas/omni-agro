"""Servidor FastAPI — OMNI Agro (conteo de plantas, malezas, despoblamiento).

Rutas:
  GET  /                     -> dashboard
  GET  /api/videos           -> lista de videos + config
  GET  /api/video/{name}/meta-> metadatos (fps, frames, duración)
  GET  /api/video/{name}/frame -> primer frame JPEG (para el editor de ROI)
  GET  /api/video/{name}/zones -> ROI guardado
  POST /api/video/{name}/zones -> guarda ROI
  POST /api/start            -> inicia procesamiento {video, conf, detector}
...
app = FastAPI(title="OMNI Agro — MVP dron")


@app.on_event("startup")
def _warmup():
    """Precarga el detector en segundo plano para eliminar el arranque en frío."""
    import threading
    ...
    def _load():
        ...
        for kind in dict.fromkeys([config.detector, "agro", "world"]):
            try:
                d = get_detector(kind)
...
def list_videos():
    ...
    return {"videos": vids, "default_conf": config.default_conf,
            ...
            "gap_alert_pct": config.gap_alert_pct,
            "weed_alert_pct": config.weed_alert_pct}
...
async def start(request: Request):
    ...
    show_grid = bool(body.get("show_grid", True))
    class_mode = body.get("class_mode") or config.class_mode
    p = _vpath(name) if name else None
    if not p or not p.exists():
        return JSONResponse({"error": "video no encontrado"}, status_code=404)
    try:
        processor.start(str(p), name, conf, detector, show_grid, class_mode)
    except Exception as e:
